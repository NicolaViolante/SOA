...
LIST_HEAD(target_uids_list);
int num_target_uids = 0;

LIST_HEAD(target_progs_list);
int num_target_progs = 0;

int throttler_add_uid(uid_t uid) {
    struct target_uid_node *new_node;
...
int throttler_remove_uid(uid_t uid) {
    struct target_uid_node *curr, *tmp;
    unsigned long flags;
    bool found = false;

    spin_lock_irqsave(&config_lock, flags);

    list_for_each_entry_safe(curr, tmp, &target_uids_list, list) {
        if (curr->uid == uid) {
...
int throttler_add_prog(const char *prog) {
    ...
    strscpy(tmp_name, prog, TASK_COMM_LEN);
    new_node = kmalloc(sizeof(*new_node), GFP_KERNEL);
    if (!new_node) {
        return -ENOMEM;
    }
    strscpy(new_node->prog, tmp_name, TASK_COMM_LEN);
    spin_lock_irqsave(&config_lock, flags);
    list_for_each_entry(curr, &target_progs_list, list) {
        if (strncmp(curr->prog, tmp_name, TASK_COMM_LEN) == 0) {
            spin_unlock_irqrestore(&config_lock, flags);
            kfree(new_node); 
            return 0;
        }
...
int throttler_remove_prog(const char *prog) {
    struct target_prog_node *curr, *tmp;
    unsigned long flags;
    bool found = false;
    char tmp_name[TASK_COMM_LEN];
    ...
    list_for_each_entry_safe(curr, tmp, &target_progs_list, list) {
        if (strncmp(curr->prog, tmp_name, TASK_COMM_LEN) == 0) {
            list_del_rcu(&curr->list);
            num_target_progs--;
            found = true;
