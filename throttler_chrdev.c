...
int throttler_chrdev_init(void) { //allocazione dinamica majoe e minor
    ...
    if (err) {
      pr_err("%s-DEV: Cannot register char device %s, got error %d\n",MODNAME, DEVICE_NAME, err);
      return err;
    }
    ...
    if (err) {
      pr_err("%s-DEV: Cannot add device %d:%d, got error %d\n",MODNAME, MAJOR(device.dev), MINOR(device.dev), err);
    ...
    if (IS_ERR(device_class)) {
        ...
        pr_err("%s-DEV:Cannot create class %s, got error %d\n",MODNAME, DEVICE_CLASS, err);
    ...
    if (IS_ERR(dp)) {
        err = PTR_ERR(dp);
        pr_err("%s-DEV: Cannot create device /dev/%s, got error %d\n",MODNAME,DEVICE_NAME, err);
        goto out3; // pulisce tutto
    }
    
    pr_info("%s-DEV: Device /dev/%s successfullty initialized!\n", MODNAME,DEVICE_NAME);
    return 0;
